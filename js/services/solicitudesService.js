...
async function procesarRespuesta(response, mensajeError) {
    if (!response.ok) {
        let detalle = "";
        try { detalle = await response.text(); } catch (error) { console.error("No se pudo leer el detalle de la respuesta.", error); }
...
export const solicitudesService = {
    getAll: obtenerSolicitudes,
    getById: obtenerSolicitud,
    crear: crearSolicitud,
    actualizar: actualizarSolicitud,
    getZonas: obtenerZonas,
    crearZona,
    crearDecision
};
